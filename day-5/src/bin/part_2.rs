...
fn sort_line(line: Vec<usize>, pairs: HashMap<usize, Nums>) -> Vec<usize> {
    ...
    for i in 0..line_len - 1 {
        for j in 0..line_len - 1 - i {
            ...
            // println!(
            //     "elem: {:?}, next_elem: {:?}, elem_nums: {:?}",
            //     elem, next_elem, elem_nums
            // );

            if elem_nums.after.contains(&next_elem) {
                // println!("\tnext_elem: {:?} is in incorrect position", next_elem);
                new_line.swap(j, j + 1);
            }
        }
    }
...
53|13";
    ...
    fn test_sort_line(#[case] input: Vec<usize>, #[case] expected: Vec<usize>) {
        // given
        let pairs = "47|53
97|13
97|61
...
53|13";
        ...
        // when
        let output = sort_line(input, pairs);
